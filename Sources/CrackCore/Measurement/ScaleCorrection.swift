...
extension CrackMeasurement {
    /// 縮尺を `factor` 倍にした写し。幅・延長・mm/px が変わり、芯線の画素座標は変わらない。
    /// 既に測った候補に、後から当てた縮尺補正を反映するのに使う。
    public func scaled(by factor: Double) -> CrackMeasurement {
        ...
        let scaledSamples = samples.map { s in
            WidthSample(
                ...
                // 補正前の半値幅は画素の量なので縮尺では変わらない。**落とさない**
                // （後から幅校正を当て直すのに要る）
