...
| | |
|---|---|
| **その場で幅を測る** | 「計測」で静止画を撮り、亀裂を指でなぞると、その区間の幅・延長を mm で表示。枠内の自動検出もある |
| **撮影ガイド** | 分解能（mm/px）・距離・入射角を常時表示。目標幅を測れない位置では具体的に「◯m まで近づいて」と指示 |
| **撮り逃し防止** | ピンボケ・白飛び・急角度・トラッキング不安定を検出して撮影をブロック。手ブレは計測前に静止を待ち、撮った画像のブレを注記 |
...
3. **枠を壁に向ける** — HUD の分解能が緑になるまで近づく。枠はピンチで大きさ、
   ドラッグで位置を変えられ、枠の下に実寸（mm）が出ます
4. **「計測」を押す** — 動きが収まるのを待って高解像度の静止画を撮り、その静止画が
   全画面に出ます（手ブレしていれば注記が出ます）
5. **亀裂を 1 本の指でなぞる** — なぞった線の近くにある亀裂を、なぞった区間に限って
   測ります。2 本指で移動、ピンチで拡大。何本でもなぞれます
6. **縮尺を合わせる（絶対幅を出すときは必ず）** — 亀裂の横に既知の長さ（100 mm の
   目印を付けたテープなど）を写し込み、「縮尺」で両端に画面中央の照準を合わせて長さを入れます。
   LiDAR の距離は 0.3 m で 3〜10% 揺れる（実測）ので、そのままだと 2 mm の亀裂で
   ±0.1 mm ずれます。目印で合わせれば、その静止画の中では消えます
7. **幅を合わせる（案件で一度）** — クラックスケールなど**実幅が分かっている線**を
   なぞって測り、その線を選んで「幅校正」に実際の幅を入れます。太さの違う 2 本を
   入れると、ボケ（σ）と一定の太りが分かれて決まります。校正は案件に残り、次の
   撮影から効きます。既に測った線も撮り直さずに測り直されます
8. **記録するものを選んで「記録」** — 線かチップをタップで選択。計測に使った静止画も
   証拠として保存され、縮尺補正の倍率と幅校正の値も記録に残ります
9. **案件画面から CSV / PDF を書き出す**

「枠内を全部測る」ボタンもありますが、暗くて細長いものは全部候補になります
（目地・斑点・影の縁）。明らかなゴミは落とし（10 mm 未満・幅の 8 倍未満の長さ・
低い信頼度）、残りは既定で選択しません。主役はなぞる方です。

画面の枠は**実際に解析する範囲**です。枠を画像座標へ写し、解析できる画素数の上限で
中心から縮めた結果を描いています。「全部測る」は枠の中だけを見ます。

### 実機で確かめる（現地の前に）

幅が分かっている線を測って、自分の端末で何 mm がいくらと出るかを一度見てください。
ずれていたら「幅校正」で合わせます（コードの数値は触りません）。
